...
public: // DEBUG make all variables public to enalbe debugger
	...
	char det_zone(double *coeff)
	{
		...
		else if (coeff[1] == 1)
		{
			...
			else // Stationary. Follows f(t)=t.
				if (coeff[2] < 0)
				{
					zn = 'f';
				}
				else // Forward, defined by the sign of the Quadratic degree.
					if (coeff[2] > 0)
					{
						zn = 'b';
					}; // Backwards, defined by the sign of the Quadratic degree.
		}
		else if (coeff[1] < 1)
		{
			zn = 'f';
	...
	void refresh_state(long PNt)
	{
		...
		case 0: // Tau is CONSTANT segments
			...
			if (ty_Bck == INF)
			{	// Special case: Delay older than oldest segment.
				// Calculates new Segment from the Initial Position
				advance_time(y, -ty_Fwd, -1);
			} // Extrapolates backwards ("invents" a piece of segment).
			else
			{	// No special case
				// Calculates new Segment from the Initial Position
				advance_time(y, ty_Bck, -1);
			}; // Advances the segment forward in time, since its beggining, ty_Bck units ahead
			break;
		...
		case 1: // Tau is LINEAR segments
			...
			{ // Tau is moving FORWARDS
				...
				{
					if (ty_Fwd == INF)
					{	// Special case: This is the Last Segment in tue queue
		...
		case 2: // Tau is PARABOLIC segments
			...
			{ // Tau is moving BACKWARDS
				...
				{ // Special case: Delay older than oldest segment.
					if (ttau_Inv == INF)
					{
						tnext = INF; // Tau does not change direction
						nxt = 'i';
					}
					else
					{
						tnext = ttau_Inv; // Tau direction change will predominate
						nxt = 't';
					};
					// Adjust Initial Position (Find target Instant inside target Segment)
					advance_time(y, -ty_Fwd, -1); // Extrapolates backwards ("invents" a piece of segment backwards in time).
				...
				{ // No special case
					...
					if (ty_Nxt < ttau_Inv)
					{ // Change in Y will occur before a change in Tau
						tnext = ty_Nxt;
						nxt = 'y';
					};
					if (ty_Nxt > ttau_Inv)
					{ // Direction change in Tau will occur before a change in Y
						tnext = ttau_Inv;
						nxt = 't';
					};
					if (ty_Nxt == ttau_Inv)
					{					  // Change in Y will occur at the same time as Tau
			...
			{ // Tau is moving FORWARDS
				...
				{	// Special case: Delay older than oldest segment.
					...
					if (ty_Fwd > ttau_Inv)
					{ // Change in Tau will occur before a change in Y
						tnext = ttau_Inv;
						nxt = 't';
					};
					if (ty_Fwd == ttau_Inv)
				...
				{
					if (ty_Fwd == INF)
					{	// Special case: This is the Last Segment in que queue
						// ---------- printLog("      	(enter f inf) \n"); // DEBUG
						if (ttau_Inv == INF)
						{
							tnext = INF; // Tau does not change direction
						...
						{
							tnext = ttau_Inv; // Tau will predominate
